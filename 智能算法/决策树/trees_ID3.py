...
def Giniimpurity(dataSet):
    ...
    for cur in dataSet:
        curclass=cur[-1]
        if curclass not in classDir.keys():
            classDir[curclass]=0
        classDir[curclass]+=1
    Ent=0.0
    for key,value in classDir.items():
        pro=1.0*value/tot
        Ent+=1.0*pro*(1-pro)
    return Ent
#简约版
def Giniimpurity1(dataSet):
...
def chooseBestFeatureToSplit(dataSet):
    ...
    baseEntropy=calcshannonEnt1(dataSet)#保存最初的原始香农熵作为最初无序度量值
    bestInfoGain=0.0
    bestFeature=-1
    for i in range(numFeatures):#遍历所有特征
        ...
        for value in uniqueVals:#遍历当前特征中的所有唯一属性值
            ...
            newEntropy+=prob*calcshannonEnt1(subDtaSet)#对所有唯一特征值得到的熵值求和
...
def createDataSet():
    dataSet=[[1,1,'yes'],
             ...
             [1,0,'maybe'],
             [0,1,'no'],
             [0,1,'no']]#不浮出水面是否可以生存；是否有脚蹼；属于鱼类
    labels=['不浮出水面可以生存','有脚蹼']#['no surfacing','flippers']
...
def classify(inTree,featLables,testVec):
    ...
    featIndex=featLables.index(firstStr)#查找标签列表中第一个匹配firstStr变量的元素
    classLabel=''
    for key in secondDict.keys():
...
if __name__=='__main__':
    ...
    print(calcshannonEnt1(myDat))
    print(Giniimpurity(myDat))
    print(Giniimpurity1(myDat))
