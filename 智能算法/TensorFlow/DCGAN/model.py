...
class DCGAN(object):
    def __init__(self, sess, image_size=64, is_crop=False,
        ...
        Args:
            sess: 会话
            batch_size: 每一批大小.训练前确定.
            lowres: (可选) 低分辨率图像/缩小因子. [8]
            z_dim: (可选) Z维数. [100]
            gf_dim: (可选) G网络第一个卷积层卷积核深度维数. [64]
            df_dim: (可选) D网络第一个卷积层卷积核深度维数. [64]
            gfc_dim: (可选) G网络全连接层维数. [1024]
            dfc_dim: (可选) D网络全连接层维数. [1024]
            c_dim: (可选) 图像颜色通道数. [3]
        """
        # 图像大小为>=8的2的幂.
        ...
        self.sess = sess #定义会话
        self.is_crop = is_crop #是否需要裁剪
        self.batch_size = batch_size #定义批大小
        ...
        self.lowres_shape = [self.lowres_size, self.lowres_size, c_dim] #缩小lowres倍的图像大小（低分辨率）

        self.z_dim = z_dim #G网络初始输入的z向量（噪声或者已知概率分布的一个采样序列）大小
        ...
        self.lam = lam #上下文和感知的损失之和的权重（数值较小）

        self.c_dim = c_dim

        # 批标准化 : 初始化处理，有利于之后的梯度处理
        self.d_bns = [batch_norm(name='d_bn{}'.format(i,)) for i in range(4)] #D网络批标准化

        log_size = int(math.log(image_size) / math.log(2)) #图像大小的2的幂次
        self.g_bns = [batch_norm(name='g_bn{}'.format(i,)) for i in range(log_size)] #G网络批标准化

        self.checkpoint_dir = checkpoint_dir #检查点目录
    ...
    def discriminator(self, image, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            h0 = lrelu(conv2d(image, self.df_dim, name='d_h0_conv'))#第一层卷积，self.df_dim默认64，image_size默认64
            #64*64*3->32*32*64
            h1 = lrelu(self.d_bns[0](conv2d(h0, self.df_dim * 2, name='d_h1_conv'), self.is_training))#第二层卷积
            #32*32*64->16*16*128
            h2 = lrelu(self.d_bns[1](conv2d(h1, self.df_dim * 4, name='d_h2_conv'), self.is_training))#第三层卷积
            #16*16*128->8*8*256
            h3 = lrelu(self.d_bns[2](conv2d(h2, self.df_dim * 8, name='d_h3_conv'), self.is_training))#第四层卷积
            #8*8*256->4*4*512
            h4 = linear(tf.reshape(h3, [-1, 8192]), 1, 'd_h4_lin')#线性层，上一层输出拉成向量，输出1个值

            return tf.nn.sigmoid(h4), h4 #输出层，sigmoid函数转为0~1的值
    ...
    def generator(self, z):
        ...
        with tf.variable_scope("generator") as scope:
            ini_depth_mul=16
            ini_size=4
            self.z_, self.h0_w, self.h0_b = linear(z, self.gf_dim * ini_depth_mul* ini_size* ini_size, 'g_h0_lin', with_w=True)
            #线性函数，z维列向量通过线性变换转为gf_dim * 16 * 4 * 4维

            hs = [None]
            hs[0] = tf.reshape(self.z_, [-1, ini_size, ini_size, self.gf_dim * ini_depth_mul])#[batch_size，4,4,gf_dim(64)*16]，即4*4*1024
            hs[0] = tf.nn.relu(self.g_bns[0](hs[0], self.is_training))

            i = 1  # 当前迭代次数
            depth_mul = 8  # 随着迭代增加，每次深度减少
            size = 8  # 随着每次深度减小，大小增加

            while size < self.image_size:#生成图像大小小于实际图像大小
                '''4*4*1024->8*8*512->16*16*256->32*32*128->64*64*64'''
                hs.append(None)
                name = 'g_h{}'.format(i)
    ...
    def build_model(self):
        '''建立GAN网络'''
        self.is_training = tf.placeholder(tf.bool, name='is_training') #区分训练和预测
        ...
        self.z_sum = tf.summary.histogram("z", self.z)#用来显示直方图信息
        ...
        self.lowres_G = tf.reduce_mean(tf.reshape(self.G,
            [self.batch_size, self.lowres_size, self.lowres,
             self.lowres_size, self.lowres, self.c_dim]), [2, 4])
        #建立两个D网络，共享参数
        self.D, self.D_logits = self.discriminator(self.images)#输入为训练数据
        self.D_, self.D_logits_ = self.discriminator(self.G, reuse=True)#输入为G生成图像
    ...
    def train(self, config):
        ...
        if self.load(self.checkpoint_dir):
            ...
            print("An existing model was not found in the checkpoint directory.")
